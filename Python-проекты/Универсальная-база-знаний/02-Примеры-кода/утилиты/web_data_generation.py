...
def _setup_logger() -> logging.Logger:
    """
    Настраивает и возвращает логгер для модуля web_data_generation.

    Returns:
        Настроенный логгер для модуля web_data_generation
    """
    ...
    log_file = logs_dir / "web_data_generation.log"
...
def home_page(date_time: str, transactions: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        # Импорт функций из других модулей bank-code
        from .date_operations import format_date, get_month_start
        from .financial_api import get_currency_rates, get_stock_prices
        from .data_loading import load_user_settings
        ...
        df_filtered = transactions[
            ...
            & (transactions["Статус"] == DEFAULT_STATUS)
        ].copy()
